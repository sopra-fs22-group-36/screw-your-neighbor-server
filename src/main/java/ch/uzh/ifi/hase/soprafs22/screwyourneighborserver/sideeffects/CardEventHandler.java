...
public class CardEventHandler {
    ...
    public CardEventHandler(RoundRepository roundRepo, CardRepository cardRepo, ParticipationRepository participationRepo) {
    ...
    public void handleAfterSave(Card card) {
        ...
        if (currentRound != null) {
            // check if every player has played his card i.e. as many cards are assigned to the current round as we
            // have players in the game
            int numberOfPlayers = currentRound.getMatch().getGame().getParticipations().size();
            int numberOfPlayedCards = cardRepo.countByRoundId(currentRound.getId()).intValue();
